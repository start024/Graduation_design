...
def fetch_nvd_llm_vulnerabilities():
    print(f"[{datetime.datetime.now()}] 🚀 启动 NVD 权威漏洞库同步程序...")
    ...
    if NVD_API_KEY != "PLEASE_FILL_IN_YOUR_API_KEY_HERE":
        ...
        print("✅ 已加载 NVD API Key 获取高优先级访问权。")
    else:
        print("⚠️ 未配置 API Key，正在使用匿名限流模式（每 30 秒 5 次）。为了稳定运行，请务必填入 Key。")
        ...
        for keyword in keywords:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"⚠️ 请求 NVD 失败，状态码: {response.status_code}。可能触发限流。")
                
            except Exception as e:
                print(f"❌ 检索 '{keyword}' 时发生网络错误: {e}")
            
            # API 礼仪：NVD 规定哪怕有 key，也最好控制请求速率
            print("⏳ 强制休眠 6 秒，防止被 NIST 封锁 API Key...")
            time.sleep(6)

    print(f"\n🎉 权威数据采集结束！系统总共新增 {total_added} 条国家漏洞记录。")
